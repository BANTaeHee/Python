'''
    1. 경사하강법
        1) 역전파 알고리즘은 신경망 학습 문제를 최적화 문제(optimization)에 접근함.
        ...
        3) 손실함수 (loss, cost function)을 최소화 하기 위해 함수(f)의 계수(coefficient) 값을 찾기 위해 사용되는 최적화 알고리즘
            - 계수의 초기 추정값을 시작으로 계속해서 계수를 업데이트하며,
              손실(cost)을 줄이는 일련의 작없을 수항하는 알고리즘.
