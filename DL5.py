'''
    1. 퍼셉트론(perceptron)
        ...
        3) 딥러닝은 은닉층이 2개 이상인 인공신경망 모형을 학습하는 것임.
        4) 은닉층과 출력층
            - 각 입력값을 처리하는 합성함수와 출력으로 보내기 위한 활성화함수로 구성

    2. 역전파(Backpropagation)
        1) 가중치와 편향을 학습하기 위한 방법으로 고안
        2) 신경망의 오차(예측값과 실제값의 차이)를 출력층에서 입력틍으로 거꾸로
           전파시켜 각 층(layer)의 가중치와 편향을 업데이트함
        3) 신경망이 깊어질수록 학습력이 좋야져야 하는데 기대하는 결과가 나오지 않는 일이 지속적 발생
...
def perceptron_fit(X, Y, epochs=10):
    ...
    for t in range(epochs):
        ...
        for i in range(len(X)):
            ...
            error = Y[i] - predict      # 오차 계산 (학습 10번하면서 오차 변화를 확인하기 위함)
            W += eta * error * X[i]
